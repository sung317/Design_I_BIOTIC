...
switch method
  ...
  case{'Biexp'}
    data = abs(data);
    nbrOfFitParams = 7; % Number of output arguments for the fit
    fitStr = ['[T1L T1S ctheta1 ctheta2 M0L M0S res] = T1BiexpFit(data(jj,:),nlsS);']
    storeStr = 'll_T1(jj,:) = [T1L T1S ctheta1 ctheta2 M0L M0S res];'
    clearStr = 'clear jj T1L T1S ctheta1 ctheta2 M0L M0S res'
end
