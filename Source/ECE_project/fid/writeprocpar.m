...
if nargin < 2
   error('WRITEPROCPAR: Invalid arguments, requires path and parameter set.');
end
...
fp = fopen([fpath '/procpar'], 'r+', 'ieee-be');
if fp == -1
   fp = fopen([fpath '.fid/procpar'], 'r+', 'ieee-be');
   if fp == -1
      error(['SETPARAM unable to open ' fpath '/procpar for writing.']);
   else
      fpath = [fpath '.fid'];
   end
end

fp2 = fopen([fpath '/procpar.out'], 'w+', 'ieee-be');
if fp2 == -1
   fclose(fp);
   error(['SETPARAM: Unable to create swap file ' fpath '/procpar.out']);
end
...
while (buffer ~= -1)
   ...
   if (findstr(value, buffer(1)))
      ...
      elseif (lenb == 15)
         if any(strcmp(cmpbuffer, {'offlineAverages'}))
            fprintf(fp2, '%s', buffer);
            fgets(fp);
            buffer = makeoutput(par.(cmpbuffer));
            fprintf(fp2, '%s \n', buffer);
...
if ~cleanup && (exist([fpath '/procpar'], 'file') == 2)
   if unix(['mv -f ' fpath '/procpar ' fpath '/procpar.orig'])
      error('Unable to save original %s/procpar', fpath);
   end
end
%-------------------------------------------------------
if unix(['mv -f ' fpath '/procpar.out ' fpath '/procpar'])
   error('Unable to replace original %s/procpar', fpath);
end
