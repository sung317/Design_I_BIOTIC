...
if nargin < 5
   error('WRITEFID requires five input arguments');
end
...
if ndims(k) > 2
   error('WRITEFID requires k to be two dimensional');
end
...
if fileid == -1
   error(['WRITEFID: Unable to create swap file ' fpath '/fid.out']);
end
...
try
   ...
   % performance
	...
	if fwrite(fileid, [hdr.nblocks hdr.ntraces hdr.np hdr.ebytes hdr.tbytes hdr.bbytes], 'int32') ~= 6
      error('WRITEFID: Error writing main header structure information.');
	end
	if fwrite(fileid, [hdr.vers_id hdr.status], 'int16') ~= 2
      error('WRITEFID: Error writing main header version information.');
	end
	if fwrite(fileid, hdr.nbheaders, 'int32') ~= 1
      error('WRITEFID: Error writing nbheaders field in main header.');
	end
   ...
   if fwrite(fileid, bdata(:,1), 'int16') ~= 4
      error('WRITEFID: Error writing first block header integer values.');
   end
   if fwrite(fileid, bdata(:,2:end), '4*int16', hdr.bbytes-8) ~= (hdr.nblocks-1)*4
      error('WRITEFID: Error writing block header integer values.');
   end
   
	fseek(fileid, 40, 'bof');
	if fwrite(fileid, bh.ctcount, 'int32') ~= 1
      error('WRITEFID: Error writing ctcount field in first block header.');
	end
	if fwrite(fileid, bh.ctcount(ones(hdr.nblocks-1,1)), 'int32', hdr.bbytes-4) ~= hdr.nblocks-1
      error('WRITEFID: Error writing ctcount field in block headers.');
	end
   ...
   if fwrite(fileid, bdata(:,1), 'float32') ~= 4
      error('WRITEFID: Error writing first block header float values.');
   end
   if fwrite(fileid, bdata(:,2:end), '4*float32', hdr.bbytes-16) ~= (hdr.nblocks-1)*4
      error('WRITEFID: Error writing block header float values.');
   end
   clear bdata;
   
	% Goto the beginning of the data
	if fseek(fileid, 32, 'bof') == -1
      error('WRITEFID: Failure to move to start of data location.');
	end
...
if numel(k) ~= count
   error('WRITEFID: Failure to write data.');
end
...
if ~cleanup && (exist([fpath '/fid'], 'file') == 2)
   if unix(['mv -f ' fpath '/fid ' fpath '/fid.orig'])
      error('Unable to save original %s/fid', fpath);
   end
end
%-----------------------------------------------------
if unix(['mv -f ' fpath '/fid.out ' fpath '/fid'])
   error('Unable to replace original %s/fid', fpath);
end
